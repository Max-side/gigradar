...
寫於 2026-09-15。這份文件的目的：讓一個完全沒看過這個對話紀錄的人（包含未來的你，或另一台電腦上全新開的 Claude Code session）能在 5 分鐘內知道現在做到哪、能不能信任目前的程式碼、下一步該做什麼。
...
## 現在的狀態：M1~M5 完成，都在瀏覽器裡實測過，不是只寫完沒測
...
| M6 | 新上架頁讀 `data/digest.json` | ❌ 尚未開始 |
| M7 | 手動新增場次的實際存檔邏輯（表單目前只是靜態 UI） | ❌ |
...
M6（新上架頁）是最小、最獨立的下一塊，適合接續。如果想先看到「真的有資料」的畫面，先做上面提到的「補幾筆 artists.yml」那件事，會讓後續每個里程碑的手動測試都輕鬆很多。
