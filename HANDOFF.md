...
**同時發現一個跟這次改動無關、範圍更大的既有缺口，先記錄、沒有動手做**：首頁上方「全部城市」「10月」「價格」三個篩選 chip（`index.html` 的 `.filter-row`）是完全沒接上任何邏輯的靜態裝飾，點下去沒有任何反應。底層的篩選邏輯其實已經寫好也測過——`src/filter.js` 的 `passesViewFilters(event, viewFilters)` 支援 `city`/`month`/`priceMax`——但 `app.js` 從來沒有把任何 UI 互動接到這個函式的 `viewFilters` 參數上。要做的話是一塊完整的小功能（要決定點擊 chip 後長怎樣：下拉選單？彈出選單？），不是一行修復，留給 Max 決定要不要現在做。
