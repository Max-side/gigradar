...
`artists.yml` 補完之後，`events.json` 第一次有了真的資料（244 筆），現在最值得做的事情反而是**打開網頁真的滑一輪，看看畫面對不對**——這麼多筆新資料、這麼多次解析邏輯的改動，還沒有人用眼睛實際檢查過時間表頁面長什麼樣子。特別留意：多主秀拼盤場次的標籤顯不顯示正常、`city: 未知` 的那 50 筆會不會讓「全部城市」篩選器出現一個很奇怪的空白選項、價格顯示（iNDIEVOX/FANSI GO 沒有價格資料，`price_min/max` 都是 `null`）畫面上會怎麼呈現。

其次：
- **`data/artists.yml` 的草稿還沒 commit**——先讓 Max 用 `git diff data/artists.yml` 看過一輪，有問題現在改最容易，之後這份檔案会越補越大，回頭校對的成本只會更高。
- 剩下 64 筆待整理，大多是非音樂雜訊（用「忽略」按鈕清掉即可）或真的看不出主秀的拼盤場次，不需要特別處理。
- 52% 左右的覆蓋率抽樣可以視為現階段用免費工具、五個來源都做完後的實際天花板，再往上要嘛擴大追蹤場館清單（投報率遞減，前面查證過大多數自營小場館很難批次找到），要嘛是接受這個範圍——不建議現在就投入。
