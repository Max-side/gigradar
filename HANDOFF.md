...
**離 1 分鐘內還有一點差距，卡在拓元**：這次測試裡拓元自己就花了 2 分 16 秒（79 場裡有 20 場當天是「新的」，需要真的開 Playwright 分頁）——這 20 場不應該要花超過 2 分鐘（用 800ms 延遲估計正常應該在 1 分鐘內），跟前面「M12 之後」章節記錄的拓元詳情頁抓取速度不穩定是同一個還沒查清楚原因的問題。**正常情況下**（一天新增的場次是個位數，不是 20 場這麼多）應該能壓到 1 分鐘內，但拓元本身這個抓取速度不穩定的問題還沒解決，值得之後再觀察。
