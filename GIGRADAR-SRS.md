...
| 相關文件 | `GIGRADAR-SPEC.md`（技術架構與實作規格） |
...
- 資料來源改版導致場次 ID 變動時，已排除項目**不得**重新出現（ID 穩定性要求，見 `GIGRADAR-SPEC.md` §3、§4）。
...
> 註：FR-52／FR-53 需要伺服器端知道使用者的偏好清單，牽涉偏好同步（見 `GIGRADAR-SPEC.md` §9 Phase 3），故列 P2。
...
| FR-65 | 偏好自動同步至 private Gist，手機與桌機保持一致 | **P0** |
...
- Given 同步用的 Gist 無法存取（token 失效或離線），When 開站，Then 仍以本機 localStorage 正常運作，並顯示同步失敗提示，**不得**因此清空本機偏好。
...
> **與 v0.1 的差異**：FR-17（手動新增場次）與 FR-65（Gist 同步）自 Phase 2 提前至 Phase 1。前者是社群平台獨家資訊的唯一補救管道，後者是因為每日快掃在手機、週末規劃在桌機，跨裝置同步屬核心情境而非加分項。
...
| # | 決策 | 結論 |
|---|---|---|
| D12 | 偏好同步 | **Phase 1 即實作 Gist 同步**（原規劃 Phase 2，已提前，理由見 §8） |
| D13 | repo 公開性 | **public repo + 免費 GitHub Pages**；偏好資料存 **private Gist**，不隨 repo 公開 |
...
| R4 | localStorage 遺失 | 中 | 高（長期資產歸零） | FR-63/64 備份提示；Phase 2 Gist 同步 |
...
| 怎麼做（資料模型、pipeline、演算法、repo 結構） | `GIGRADAR-SPEC.md` | §2–§12 |
...
|---|---|---|---|---|
| 1 | **定義需求與架構** ✅ | 本文件 SRS v1.0 + `GIGRADAR-SPEC.md` | — | ~~§9 決策拍板，SRS 升版 v1.0~~ **已完成** |
...
| 4 | **開發** | 依 `GIGRADAR-SPEC.md` §11 分階段實作 | 設計稿確認 | Phase 1 全部 AC 通過，**特別是 AC-42／AC-44** |
